...
import { Settings, Plus, Trash2, Save, Pencil, X, Clock, Package } from "lucide-react";
...
export default function CatalogManager() {
  ...
  const create = async () => {
    ...
    setCreating({ category: "producto", name: "", price: 0, delivery_minutes: 30 });
    load();
  };
  ...
  return (
    <div className="grid gap-6">
      <div className="rounded-lg border bg-card">
        ...
        <div className="p-5 grid sm:grid-cols-6 gap-3">
          ...
          <button onClick={create} className="rounded-md bg-primary text-sm font-semibold text-primary-foreground hover:bg-primary/90">
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <div className="border-b px-5 py-3 flex items-center gap-3 flex-wrap">
          ...
          <select value={filter} onChange={(e) => setFilter(e.target.value)} className="cm-input ml-auto !w-auto text-xs">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y">
                  ...
                  if (ed) {
                    return (
                      <tr key={s.id} className="bg-primary/5">
                        <td className="px-4 py-2"><select value={ed.category} onChange={(e) => setEditing({ ...editing, [s.id]: { ...ed, category: e.target.value } })} className="cm-input">{CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></td>
                        <td className="px-4 py-2"><input value={ed.name ?? ""} onChange={(e) => setEditing({ ...editing, [s.id]: { ...ed, name: e.target.value } })} className="cm-input" /></td>
                        <td className="px-4 py-2"><input type="number" value={ed.price ?? 0} onChange={(e) => setEditing({ ...editing, [s.id]: { ...ed, price: Number(e.target.value) } })} className="cm-input text-right" /></td>
                        <td className="px-4 py-2"><input type="number" value={ed.delivery_minutes ?? 30} onChange={(e) => setEditing({ ...editing, [s.id]: { ...ed, delivery_minutes: Number(e.target.value) } })} className="cm-input text-right" /></td>
                ...
                {filtered.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-muted-foreground">No hay ítems en esta categoría.</td></tr>}
              </tbody>
            </table>
          </div>
        )}
